...
export default function SustainabilitySpotlight() {
  ...
  const animateLayer = (
    ...
    tl.to(layerB, { clipPath: "inset(0% 0% 0% 0%)", duration: 1.0, ease: "expo.inOut" });
  ...
  const go = useCallback(
    (nextIndex: number, forcedDirection?: 1 | -1) => {
      ...
      animateLayer(dLayerARef.current, dLayerBRef.current, dLayerBImgRef.current, dir, prevSrc, nextSrc, onComplete);
      animateLayer(mLayerARef.current, mLayerBRef.current, mLayerBImgRef.current, dir, prevSrc, nextSrc, onComplete);
    },
    [isAnimating, slides],
  );
  ...
  const handlePointerDown = (e: React.PointerEvent) => { dragStartX.current = e.clientX; isDragging.current = false; };
  const handlePointerMove = (e: React.PointerEvent) => { if (dragStartX.current === null) return; if (Math.abs(e.clientX - dragStartX.current) > 5) isDragging.current = true; };
  const handlePointerUp = (e: React.PointerEvent) => {
    if (dragStartX.current === null) return;
    const delta = e.clientX - dragStartX.current;
    if (isDragging.current && Math.abs(delta) >= DRAG_THRESHOLD) { delta < 0 ? goNext() : goPrev(); }
    dragStartX.current = null; isDragging.current = false;
  };
  const handlePointerLeave = () => { dragStartX.current = null; isDragging.current = false; };
      ...
      <div className="container">
        ...
        <div className="flex flex-col items-center lg:hidden">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={slide.id} className="flex flex-col items-center text-center mb-50">
              <motion.span custom={0} variants={itemVariants} initial="hidden" animate="visible" exit="exit" className="text-16 font-[avenirHeavy] text-foreground-light mb-20">
                {slide.date}
              </motion.span>
              <motion.h3 custom={1} variants={itemVariants} initial="hidden" animate="visible" exit="exit" className="text-25 font-[optima] uppercase text-foreground leading-[1.2] mb-50 max-w-[598px]">
                {slide.title}
              </motion.h3>
              <motion.div custom={2} variants={itemVariants} initial="hidden" animate="visible" exit="exit">
                <Link href={slide.href} className="text-primary-2 text-16 font-[avenirHeavy] hover:opacity-70 transition-opacity duration-300">
                  Read More...
                </Link>
              </motion.div>
            </motion.div>
          ...
          <div className="flex items-center gap-[10px] mb-50">
            {slides.map((_, i) => (
              <button key={i} onClick={() => go(i)} className={`rounded-full transition-all duration-300 cursor-pointer w-[10px] h-[10px] ${i === current ? "bg-primary-2" : "bg-white border border-primary-2"}`} />
            ))}
          </div>

          {/* Mobile image — own refs */}
          <div className="w-full relative h-[320px] sm:h-[400px] overflow-hidden cursor-grab active:cursor-grabbing select-none mb-50" {...pointerHandlers}>
            <div ref={mLayerARef} className="absolute inset-0">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={slides[0].image} alt={slides[0].alt} className="absolute inset-0 w-full h-full object-cover object-center" draggable={false} />
            </div>
            <div ref={mLayerBRef} className="absolute inset-0" style={{ clipPath: "inset(0% 100% 0% 0%)" }}>
          ...
          <div className="flex justify-center gap-30">
            <CustomOutlineButton text="View All" borderColor="border-primary-2" textColor="text-foreground-light" px="px-[18px] sm:px-[26px]" />
        ...
        <div className="hidden lg:block">
          <div className="flex items-center gap-50 3xl:gap-0">
            ...
            <div className="w-1/2 relative h-[460px] 3xl:h-[602px] overflow-hidden cursor-grab active:cursor-grabbing select-none" {...pointerHandlers}>
              <div ref={dLayerARef} className="absolute inset-0">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={slides[0].image} alt={slides[0].alt} className="absolute inset-0 w-full h-full object-cover object-center" draggable={false} />
              </div>
              <div ref={dLayerBRef} className="absolute inset-0" style={{ clipPath: "inset(0% 100% 0% 0%)" }}>
          ...
          <div className="overflow-hidden">
            <div className="flex justify-center mt-50 gap-30">
              <CustomOutlineButton text="View All" borderColor="border-primary-2" textColor="text-foreground-light" px="px-[26px] lg:px-[37px]" />
              <div className="flex items-center gap-[15px]">
                <SliderArrowButton onClick={goPrev} direction="prev" variant="dark" />
                <SliderArrowButton onClick={goNext} direction="next" variant="dark" />
              </div>
