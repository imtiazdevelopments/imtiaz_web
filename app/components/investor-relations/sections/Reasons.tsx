...
export default function Reasons() {
  ...
  const [innerSlideIndices, setInnerSlideIndices] = useState<Set<number>>(new Set());
  ...
  useLayoutEffect(() => {
    const measure = () => {
      cardRefs.current.forEach((el) => { if (el) el.style.height = "auto"; });
      const maxH = Math.max(
        0,
        ...cardRefs.current.map((el) => el?.getBoundingClientRect().height ?? 0)
      );
      if (maxH > 0) {
        cardRefs.current.forEach((el) => { if (el) el.style.height = `${maxH}px`; });
      }
    };
    measure();
