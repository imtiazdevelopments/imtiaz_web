...
export default function CountryCodeSelect({
  ...
  return (
    <div ref={ref} className="relative flex-shrink-0">
      ...
      >
        <div className={`flex items-center gap-[10px] absolute -top-[27px] border-b pr-[15px] ${variant === "light" ? "border-white" : "border-foreground-light"} pb-[2px]`}>
          <span
...
<AnimatePresence>
  {open && (
    ...
    >
      {/* Search */}
      <div className={`px-3 py-2 border-b ${isLight ? "border-foreground-light/10" : "border-black/5"}`}>
        <input
          type="text"
      ...
      <div className="max-h-[220px] overflow-y-auto scrollbar-hide">
        {filtered.length === 0 && (
          <p className={`px-5 py-3 text-description ${isLight ? "text-foreground/40" : "text-foreground-light/40"}`}>
            No results
          </p>
        )}
        {filtered.map((c, i) => (
          <div key={`${c.code}-${i}`}>
            {i !== 0 && (
              <div className={`w-full h-px ${isLight ? "bg-foreground/5" : "bg-black/5"}`} />
            )}
            <button
              type="button"
              onClick={() => {
            ...
            >
              ...
              />
              <span>{c.code} - </span>
              <span className={`text-[13px] ${isLight ? "text-foreground" : "text-foreground-light"}`}>
                {c.name}
              </span>
            </button>
          </div>
