...
//               {/* ===== PILL HEADER ===== */}
//               <div className="container pt-14 md:pt-20 lg:pt-24 2xl:pt-32 relative">
...
import { textFade, moveLeft, moveRight, moveUp } from "../../motionVariants";
...
export default function HeroFeatureSlider({ slides }: Props) {
  ...
  const switchBg = async (newBg: string) => {
    if (!newBg) return;

    await preloadImage(newBg);

    setPrevBg(bgBase); // store old image
    setBgBase(newBg); // new image on top
  };
    ...
    >
                ...
                <div className="absolute w-full h-full inset-0 -z-20 overflow-hidden">
                  ...
                  {prevBg && (
                    <motion.div
                      ...
                      className="absolute w-full scale-[1.2] h-full bg-cover bg-center will-change-opacity will-change-filter"
      ...
      `}
                              >
                                <button
                                  className="inline-block border border-white px-[23px] py-[19.5px] cursor-pointer
        rounded-[50px] text-white text-17"
                                >
                                  Read More
                                </button>
                              </motion.div>
